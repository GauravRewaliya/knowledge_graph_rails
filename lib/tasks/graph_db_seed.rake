namespace :graph_db_seed do
  ...
  desc "Seed GraphQueryfier table with predefined queries"
  task seed_graph_queryfiers: :environment do
    # Find or create a default workspace
    workspace = Workspace.first || Workspace.create!(name: "Default Workspace")
    
    puts "🔄 Seeding GraphQueryfier data..."
    
    GraphQueryfier.seed_data.each do |query_data|
      query_data[:workspace_id] = workspace.id
      
      existing = GraphQueryfier.find_by(
      ...
      if existing
        existing.update!(query_data)
        puts "   ✅ Updated: #{query_data[:desc]}"
      else
        GraphQueryfier.create!(query_data)
        puts "   ✅ Created: #{query_data[:desc]}"
      end
    end
    
    puts "✅ GraphQueryfier seeded successfully with #{GraphQueryfier.count} queries"
  end

  desc "Seed both Neo4j graph and GraphQueryfier table"
  task seed_all: :environment do
    Rake::Task['graph_db_seed:question_bank_seed'].invoke
    Rake::Task['graph_db_seed:seed_graph_queryfiers'].invoke
  end
end
